...
class DBpedia:
    ...
    def _process_entity_kg(self, SELF_LOOP_ID = 185):
        ...
        self.num_relations = int(self.edge_type.max().item()) + 1
        if self.edge_type.min().item() < 0 or self.edge_type.max().item() >= self.num_relations:
            raise ValueError('Knowledge-graph relation IDs are outside the RGCN relation range.')
