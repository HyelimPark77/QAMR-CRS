...
class PromptGPT2forCRS(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if rec:
            ...
            if entity_embeds.dim() == 2:
                rec_logits @= entity_embeds.T  # (bs, n_item)
            elif entity_embeds.dim() == 3:
                rec_logits = torch.bmm(entity_embeds, rec_logits.unsqueeze(-1)).squeeze(-1)
            else:
                raise ValueError(f"entity_embeds must be 2D or 3D, got shape {tuple(entity_embeds.shape)}")
