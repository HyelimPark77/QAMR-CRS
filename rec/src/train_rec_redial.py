...
def write_evidence_log(
    log_file, split, epoch, step, router_weights, selected_evidence, labels,
    routing_beta=None,
):
    if log_file is None:
        return
    ...
    mean_router_weights = (
        router_weights.mean(dim=0).detach().cpu()
        if router_weights is not None else None
    )
    residual_adjusted_coefficients = None
    if mean_router_weights is not None and routing_beta is not None:
        beta = float(routing_beta)
        residual_adjusted_coefficients = (
            beta / mean_router_weights.numel()
            + (1.0 - beta) * mean_router_weights
        ).tolist()
    ...
    record = {
        ...
        'router_weights': mean_router_weights.tolist() if mean_router_weights is not None else None,
        'residual_adjusted_coefficients': residual_adjusted_coefficients,
...
def evaluate_only(
    split, dataloader, model, text_encoder, prompt_encoder, evaluator,
    accelerator, kg, evidence_log, query_aware_scoring, routing_beta,
):
    losses = []
    prompt_encoder.eval()
    for step, batch in enumerate(tqdm(dataloader)):
        with torch.no_grad():
            ...
            batch['context']['entity_embeds'] = prompt_encoder.get_entity_embeds_for_scoring(
                router_weights if query_aware_scoring else None
            )
            outputs = model(**batch['context'], rec=True)
            losses.append(float(outputs.rec_loss))
    ...
    result = {
        f'{split}/{key}': value / report['count']
        for key, value in report.items()
        if key != 'count'
    }
    result[f'{split}/loss'] = float(np.mean(losses))
    result['epoch'] = -1
...
def parse_args():
    ...
    parser.add_argument("--prompt_encoder", type=str,default='rec/src/pre-trained-redial/best')
    parser.add_argument("--eval_only_checkpoint", type=str, default=None,
                        help="Evaluate an existing recommendation checkpoint without training.")
    parser.add_argument("--eval_split", choices=["valid", "test"], default="valid",
                        help="Split used with --eval_only_checkpoint.")
...
if __name__ == '__main__':
    args = parse_args()
    for attr in (
        "output_dir", "tokenizer", "text_tokenizer", "model", "text_encoder",
        "prompt_encoder", "eval_only_checkpoint",
    ):
    ...
    if accelerator.is_local_main_process:
        if args.eval_only_checkpoint is not None:
            ...
            evidence_path = Path(args.output_dir) / f'evidence_{args.eval_split}.jsonl'
        else:
            evidence_path = PROJECT_ROOT / 'log' / f'evidence_rec_{local_time}.jsonl'
        evidence_log = open(evidence_path, 'w', buffering=1)
    else:
        evidence_log = None
    ...
    text_simi  = text_sim(dataset=args.dataset, pad_entity_id=kg['pad_entity_id']).get_entity_ts_info()
    image_simi = image_sim(dataset=args.dataset, pad_entity_id=kg['pad_entity_id']).get_entity_is_info()
    ...
    load_checkpoint = args.eval_only_checkpoint or args.prompt_encoder
    if load_checkpoint is not None:
        prompt_encoder.load(load_checkpoint)
    ...
    if args.eval_only_checkpoint is not None:
        prompt_encoder, valid_dataloader, test_dataloader = accelerator.prepare(
            prompt_encoder, valid_dataloader, test_dataloader
        )
        eval_dataloader = valid_dataloader if args.eval_split == 'valid' else test_dataloader
        eval_report = evaluate_only(
            ...
            query_aware_scoring=args.query_aware_scoring,
            routing_beta=args.routing_beta,
        )
        logger.info(f'{eval_report}')
        if accelerator.is_local_main_process:
            result_path = Path(args.output_dir) / f'eval_{args.eval_split}.json'
            payload = {'config': config, 'metrics': eval_report}
