...
def write_evidence_log(log_file, split, epoch, step, router_weights, selected_evidence, resp_batch):
    if log_file is None:
        return
    ...
    record = {
        ...
        'response_lengths': [len(resp) if isinstance(resp, list) else int((resp != -100).sum().item()) for resp in resp_batch],
    }
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str,default='prompt-conv-redial',help="Where to store the final model.")
    ...
    parser.add_argument("--model", type=str,default='hf_models/DialoGPT-small')
...
if __name__ == '__main__':
    args = parse_args()
    for attr in ("output_dir", "tokenizer", "text_tokenizer", "model", "text_encoder"):
    ...
    if args.use_wandb:
        ...
        run = None
    if accelerator.is_local_main_process:
        evidence_log = open(PROJECT_ROOT / 'log' / f'evidence_conv_{local_time}.jsonl', 'w', buffering=1)
    else:
        evidence_log = None
    ...
    data_collator_teacher = CRSConvDataCollator(
        tokenizer=tokenizer, device=device, use_amp=use_amp, debug=args.debug, gen=False,
    ...
    data_collator_generator = CRSConvDataCollator(
        tokenizer=tokenizer, device=device, gen=True, use_amp=use_amp, debug=args.debug,
        ignore_pad_token_for_loss=args.ignore_pad_token_for_loss,
        context_max_length=args.context_max_length, resp_max_length=args.resp_max_length,
        entity_max_length=args.entity_max_length, pad_entity_id=kg['pad_entity_id'],
        prompt_tokenizer=text_tokenizer,
    ...
    gen_file_path = str(PROJECT_ROOT / 'log' / f'gen_{local_time}.jsonl')
