...
class MMPrompt(nn.Module):
    ...
    def get_modality_entity_embeds(self):
        ...
        kg_entity_embeds = self.kg_encoder(None, self.edge_index, self.edge_type)
    ...
    def forward(self, entity_ids=None, token_embeds=None, output_entity=False, use_rec_prefix=False,
        ...
        if not output_entity:
            ...
            try:
                ...
                assert 1==0
            if modality_batch_embeds is not None:
                router_output = self.evidence_router(
                    token_hidden_states=prompt_embeds,
                    modality_evidence=modality_batch_embeds,
                    entity_summary=entity_embeds.mean(dim=1),
                )
                entity_embeds = router_output.shared
                self.last_router_weights = router_output.weights.detach()
