    ...
    def prepare_data(self, data_file):
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                for c, r in list(zip(dialog['retrieved_contexts'], dialog['retrieved_resp']))[:self.n_examples]:
                    if r == 'nan' or c == '<PAD>' or r is None:
                        continue
                    text1 = c + self.tokenizer.eos_token + r + self.tokenizer.eos_token
                    prompt_text1 = c + self.prompt_tokenizer.sep_token + r + self.prompt_tokenizer.sep_token
                ...
                data = {
                    ...
                    'resp': resp_ids,
                    'entity': list(
                        dialog['context_entities'][-self.entity_max_length:]
                        if len(dialog['retrieved_response_entity']) == 0 and len(dialog['retrieved_context_entity']) == 0
                        else (dialog['retrieved_response_entity'] + dialog['retrieved_context_entity'] + dialog['context_entities'])[-self.entity_max_length:]
                    ),
